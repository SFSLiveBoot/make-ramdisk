...
test_blockdev_uuid() {
  ...
  $kvm_cmd -drive file="$dev",if=virtio,readonly -kernel "$kernel" -initrd "$initrd" -append "root=UUID=$uuid:$share/*.sfs+:$share/\$arch/*kernel-\$kver.sfs+mem $append"
}
